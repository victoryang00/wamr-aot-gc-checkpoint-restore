...
#define ACC_AS_I32 1
#define ACC_AS_I64 2
#define ACC_AS_I32_POPPED 3
#define ACC_AS_I64_POPPED 4
#define ACC_AS_F32 5
#define ACC_AS_F64 6
#define ACC_AS_F32_POPPED 7
#define ACC_AS_F64_POPPED 8
...
static void
emit_load_acc(WASMLoaderContext *loader_ctx, uint8 type, int16 operand_offset)
{
    emit_label(EXT_OP_LOAD_ACC);
...
static void
commit_acc_for_branch(WASMLoaderContext *loader_ctx, uint8 opcode)
{
    skip_label();
    commit_acc(loader_ctx);
    emit_label(opcode);
}
        ...
        if (loader_ctx->frame_offset_bottom[i] == (int16)local_index) {
            if (!(*preserved)) {
                *preserved = true;
                ...
                inc_operand_idx(loader_ctx, local_type);
                emit_label(opcode);
            }
                             ...
                             char *error_buf, uint32 error_buf_size)
{
    ...
    if (can_pop_opnd_from_acc(ctx, type)) {
        ...
        if (is_int_type(type) && ctx->int_acc_status != ACC_UNUSED
            && ctx->int_acc_opnd_offset == *(ctx->frame_offset)
            && ctx->int_acc_is_dirty) {
            // bh_assert(0);
        }
        if (is_float_type(type) && ctx->float_acc_status != ACC_UNUSED
...
#if WASM_ENABLE_FAST_INTERP != 0
                commit_acc_for_branch(loader_ctx, opcode);
